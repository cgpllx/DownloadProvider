...
public class DownloadService extends Service {
	...
	private void updateFromProvider() {
		synchronized (this) {//cgp 20140528
			mPendingUpdate = true;
			if (mUpdateThread == null) {
				mUpdateThread = new UpdateThread();
				//cgp 2014 0528  //这个更新线程不能放在线程池中,改用start启动
	...
	private void trimDatabase() {
		Cursor cursor = getContentResolver().query(Downloads.ALL_DOWNLOADS_CONTENT_URI, new String[] { BaseColumns._ID },
				Downloads.COLUMN_STATUS + " >= '200'", null, Downloads.COLUMN_LAST_MODIFICATION);
		if (cursor == null) {
	...
	private void updateDownload(DownloadInfo.Reader reader, DownloadInfo info, long now) {
		...
		boolean lostVisibility = oldVisibility == Downloads.VISIBILITY_VISIBLE_NOTIFY_COMPLETED
				&& info.mVisibility != Downloads.VISIBILITY_VISIBLE_NOTIFY_COMPLETED && Downloads.isStatusCompleted(info.mStatus);

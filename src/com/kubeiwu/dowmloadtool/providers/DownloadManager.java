...
import com.kubeiwu.dowmloadtool.providers.downloads.Downloads;
...
public class DownloadManager {
	...
	public final static String COLUMN_VERSIONNAME = "versionname";
	public final static String COLUMN_VERSIONCODE = "versioncode";
	 ...
	 */
	public final static String COLUMN_TOTAL_SIZE_BYTES = "total_bytes";
	...
	//cgp2014 0521
	public final static String COLUMN_BYTES_DOWNLOADED_SO_FAR = "current_bytes";
	 ...
	 * completed.
	 */
	public final static int STATUS_SUCCESSFUL = 1 << 10;// << 3;//cgp2014 0520 为了方便排序
	//	public final static int STATUS_SUCCESSFUL = 1 << 3;//cgp2014 0520
	...
	private static final String[] COLUMNS = new String[] { COLUMN_ID, COLUMN_TITLE, COLUMN_DESCRIPTION, COLUMN_URI, COLUMN_MEDIA_TYPE,
			COLUMN_TOTAL_SIZE_BYTES, COLUMN_LOCAL_URI, COLUMN_STATUS, COLUMN_REASON, COLUMN_BYTES_DOWNLOADED_SO_FAR, COLUMN_LAST_MODIFIED_TIMESTAMP,
			COLUMN_ICONURL, COLUMN_VERSIONNAME, COLUMN_VERSIONCODE, COLUMN_PACKAGENAME };

	// columns to request from DownloadProvider
	private static final String[] UNDERLYING_COLUMNS = new String[] { Downloads._ID, Downloads.COLUMN_TITLE, Downloads.COLUMN_DESCRIPTION,
			Downloads.COLUMN_URI, Downloads.COLUMN_MIME_TYPE, Downloads.COLUMN_TOTAL_BYTES, Downloads.COLUMN_STATUS, Downloads.COLUMN_CURRENT_BYTES,
			Downloads.COLUMN_LAST_MODIFICATION, Downloads.COLUMN_DESTINATION, Downloads.COLUMN_FILE_NAME_HINT, Downloads._DATA,
			Downloads.COLUMN_ICONURL, Downloads.COLUMN_VERSIONNAME, Downloads.COLUMN_VERSIONCODE, Downloads.COLUMN_PACKAGENAME };

	private static final Set<String> LONG_COLUMNS = new HashSet<String>(Arrays.asList(COLUMN_ID, COLUMN_TOTAL_SIZE_BYTES, COLUMN_STATUS,
			COLUMN_REASON, COLUMN_BYTES_DOWNLOADED_SO_FAR, COLUMN_LAST_MODIFIED_TIMESTAMP, COLUMN_VERSIONCODE));
	...
	public static class Request {
		...
		public Request setVersionName(String versionName) {
			mVersionName = versionName;
			return this;
		}

		public Request setVersionCode(String versionCode) {
		...
		private void setDestinationFromBase(File base, String subPath) {
			...
			Log.i(TAG, "mDestinationUri=" + mDestinationUri.toString());
		}
		...
		ContentValues toContentValues(String packageName) {
			...
			putIfNonNull(values, Downloads.COLUMN_VERSIONNAME, mVersionName);
			putIfNonNull(values, Downloads.COLUMN_VERSIONNAME, mVersionCode);
			putIfNonNull(values, Downloads.COLUMN_PACKAGENAME, mPackageName);
			//cgp 20140519
	...
	public static class Query {
		...
		public Query orderBy(String column, int direction) {
			...
			} else if (column.equals(Downloads.COLUMN_STATUS)) {
				mOrderByColumn = Downloads.COLUMN_STATUS;
			} else {
	...
	//cgp 20140527 app的包名的接口

	public int update(String newPackageName, long... ids) {
		if (ids == null || ids.length == 0) {
			// called with nothing to remove!
			throw new IllegalArgumentException("input param 'ids' can't be null");
		}
		//return mResolver.update(mBaseUri, values, getWhereClauseForIds(ids), getWhereArgsForIds(ids));
		ContentValues values = new ContentValues();
		values.put(Downloads.COLUMN_PACKAGENAME, newPackageName);
		return mResolver.update(mBaseUri, values, getWhereClauseForIds(ids), getWhereArgsForIds(ids));
	}

	public int update(int newVersionCode, long... ids) {
		if (ids == null || ids.length == 0) {
			// called with nothing to remove!
			throw new IllegalArgumentException("input param 'ids' can't be null");
		}
		//return mResolver.update(mBaseUri, values, getWhereClauseForIds(ids), getWhereArgsForIds(ids));
		ContentValues values = new ContentValues();
		values.put(Downloads.COLUMN_VERSIONCODE, newVersionCode);
		return mResolver.update(mBaseUri, values, getWhereClauseForIds(ids), getWhereArgsForIds(ids));
	}
			...
			for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
				...
				if (status != STATUS_RUNNING && status != STATUS_PENDING) {
					throw new IllegalArgumentException("Can only pause a running download: " + cursor.getLong(cursor.getColumnIndex(COLUMN_ID)));
						...
						System.out.println("暂停出错了");
		} finally {
			cursor.close();
		}

		System.out.println("pauseDownload暂停开始");
		ContentValues values = new ContentValues();
		values.put(Downloads.COLUMN_CONTROL, Downloads.CONTROL_PAUSED);
		values.put(Downloads.COLUMN_NO_INTEGRITY, 1);
		mResolver.update(mBaseUri, values, getWhereClauseForIds(ids), getWhereArgsForIds(ids));
		System.out.println("pauseDownload暂停结束");
	}
	...
	private static class CursorTranslator extends CursorWrapper {
		...
		private String translateString(String column) {
			...
			if (column.equals(COLUMN_ICONURL)) {
				return getUnderlyingString(Downloads.COLUMN_ICONURL);
			}
			if (column.equals(COLUMN_VERSIONNAME)) {
				return getUnderlyingString(Downloads.COLUMN_VERSIONNAME);
			}
			if (column.equals(COLUMN_PACKAGENAME)) {
				return getUnderlyingString(Downloads.COLUMN_PACKAGENAME);
			}
		...
		private long translateLong(String column) {
			...
			if (column.equals(COLUMN_ICONURL)) {
				return getUnderlyingLong(Downloads.COLUMN_ICONURL);
			}
			if (column.equals(COLUMN_VERSIONNAME)) {
				return getUnderlyingLong(Downloads.COLUMN_VERSIONNAME);
			}
			if (column.equals(COLUMN_PACKAGENAME)) {
				return getUnderlyingLong(Downloads.COLUMN_PACKAGENAME);
			}
			if (column.equals(COLUMN_VERSIONCODE)) {
				return getUnderlyingLong(Downloads.COLUMN_VERSIONCODE);
			}
		...
		private String getUnderlyingString(String column) {
			//			System.out.println("column出错" + column);
